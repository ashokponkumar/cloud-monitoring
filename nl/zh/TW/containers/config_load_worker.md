...
# 在 Grafana 中配置工作者的 CPU 度量值
{: #config_load_worker}

在 {{site.data.keyword.Bluemix}} 中，會自動收集針對工作者所選取的負載度量值。若要透過 {{site.data.keyword.monitoringlong}} 進行監視，您必須定義 Grafana 查詢。
{:shortdesc}

如需自動收集的負載度量值清單，請參閱[工作者的負載度量值](/docs/services/cloud-monitoring/containers/monitoring_containers_ov.html#load_metrics_workers)。
...
3. 定義過濾圖形中所顯示資料的查詢。 
    ...
    如需查詢格式的相關資訊，請參閱[針對工作者所收集之負載度量值的查詢格式](/docs/services/cloud-monitoring/reference/metrics_format.html#load_workers)。
	...
	6. 按一下**選取度量值**來指定您要監視之工作者的工作者名稱。
	
	7. 按一下**選取度量值**來指定度量值類型，然後按一下**選取度量值**來指定度量值子類型。
	
	    如需 CPU 度量值的清單，請參閱[工作者的 CPU 度量值](/docs/services/cloud-monitoring/containers/monitoring_containers_ov.html#load_metrics_workers)。
