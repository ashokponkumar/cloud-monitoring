...
您可以將 Cloud Foundry (CF) 應用程式及虛擬機器 (VM) 的度量值傳送到 {{site.data.keyword.monitoringshort}} 服務。  

下圖顯示不同資源的高階視圖，您可以從這些資源將度量值傳送至 {{site.data.keyword.monitoringshort}} 服務，以進行分析：

![在 {{site.data.keyword.monitoringlong}} 服務中元件的高階元件概觀](images/cloud_monitoring_ov.gif)

如需如何傳送度量值的相關資訊，請參閱[將度量值傳送至 {{site.data.keyword.monitoringshort}} 服務](/docs/services/cloud-monitoring/send_retrieve_metrics_ov.html#send_retrieve_metrics_ov)。

您可以透過 {{site.data.keyword.Bluemix_notm}} 型錄存取 {{site.data.keyword.monitoringlong}} 服務。對於具有單一及群組容器的 {{site.data.keyword.containershort}}，您也可以從 {{site.data.keyword.Bluemix_notm}} 使用者介面存取服務。
...
1. **用較少的時間來檢測應用程式，而將更多的時間用在加強其價值**
	...
	{{site.data.keyword.monitoringlong_notm}} 服務每分鐘會提供一次度量值收集。精簡方案會以完整解析度保留度量值 15 天。進階方案則會以完整解析度保留度量值 45 天。
...
6. **選擇符合您需要的服務方案** 
    ...
    您可以選擇「精簡」服務方案或「進階」服務方案來符合您的用量需求。「精簡」方案提供基本平台度量值收集和補充的警示。或者，您可以選取「進階」方案，以啟用具有較長保留期間的較大度量值耗用，增加您可以定義的警示數（包括針對多個服務及應用程式報告的警示），以及取得服務 API 的存取權。
...
<table>
    <caption>表 1. {{site.data.keyword.monitoringshort}} 服務的方案摘要。</caption>
      <tr>
        <th>方案</th>
        <th>使用 API 傳送度量值</th>
        <th>度量值保留期間</th>
      ...
      <tr>
        ...
        <td>無法使用</td>
        <td>15 天</td>
        <td>您最多可以定義 10 個單一度量值警示規則或 1 個包含萬用字元的規則。</td>
      </tr>
      <tr>
        <td>進階</td>
        <td>可用</td>
...
**附註：**「精簡」方案提供與 {{site.data.keyword.Bluemix_notm}} 中整合式監視功能相同的特性。
...
會刪除過去 7 天內未收到資料的度量值。{{site.data.keyword.monitoringshort}} 服務會刪除本質上似乎是暫時性的度量值路徑的所有資料，方法為識別過去 7 天內未寫入的度量值。例如：
...
* 若要收集 {{site.data.keyword.Bluemix_notm}} 空間中執行的雲端資源的度量值，您必須在資源執行所在的相同空間中佈建服務。

* 若要將度量值傳送至 {{site.data.keyword.monitoringshort}} 服務，您必須在您要傳送度量值到其中的 {{site.data.keyword.Bluemix_notm}} 空間，佈建 {{site.data.keyword.monitoringshort}} 服務的實例。 
...
具有 {{site.data.keyword.Bluemix_notm}} 使用者 ID，並且有權可存取 {{site.data.keyword.Bluemix_notm}} 組織中空間的任何人，皆可使用 {{site.data.keyword.monitoringshort}} 服務。 
