...
5. 以下の cURL コマンドを実行して、メトリックを送信します。
	...
	* Auth_Type は、トークンのタイプを定義する接頭部です。 有効な値は、UAA トークンの場合は *uaa*、IAM トークンの場合は *iam*、API キーの場合は *api* です。
...
オプションで、関数をメトリックに適用できます。 サポートされる関数について詳しくは、[Graphite 0.9.15 functions ![外部リンクのアイコン](../../../icons/launch-glyph.svg "External link icon")](http://graphite.readthedocs.io/en/latest/functions.html) を参照してください。
