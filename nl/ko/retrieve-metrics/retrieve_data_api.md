...
5. 메트릭을 전송하려면 다음 cURL 명령을 실행하십시오.
	...
	* Auth_Type은 토큰의 유형을 정의하는 접두부입니다. 올바른 값은 UAA 토큰의 경우 *uaa*, IAM 토큰의 경우 *iam*, API 키의 경우 *api*입니다.
	...
	* *Start_Time*은 요청의 시작을 정의합니다. 이 정보는 상대 또는 절대 시간을 계산하는 데 사용됩니다. *End_Time*은 요청의 종료를 정의합니다. 이 정보는 상대 또는 절대 시간을 계산하는 데 사용됩니다. 자세한 정보는 [기간 설정](/docs/services/cloud-monitoring/retrieve-metrics/retrieve_data_api.html#time)을 참조하십시오.
...
**상태 시간** 앞에는 항상 빼기 기호(-)가 오며 그 뒤에는 시간 단위가 옵니다. 
