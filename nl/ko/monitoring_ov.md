...
Cloud Founfry(CF) 애플리케이션 및 가상 머신(VM)에 대한 메트릭을 {{site.data.keyword.monitoringshort}} 서비스에 전송할 수 있습니다.   

다음 그림은 분석을 위해 {{site.data.keyword.monitoringshort}} 서비스에 메트릭을 전송할 수 있는 다양한 리소스에 대한 상위 레벨 보기를 보여줍니다. 

![{{site.data.keyword.monitoringlong}} 서비스의 컴포넌트에 대한 상위 레벨 컴포넌트 개요](images/cloud_monitoring_ov.gif)

메트릭을 전송하는 방법에 대한 자세한 정보는 [{{site.data.keyword.monitoringshort}} 서비스에 메트릭 전송](/docs/services/cloud-monitoring/send_retrieve_metrics_ov.html#send_retrieve_metrics_ov)을 참조하십시오. 

{{site.data.keyword.Bluemix_notm}} 카탈로그를 통해 {{site.data.keyword.monitoringlong}} 서비스에 액세스할 수 있습니다. 단일 및 그룹 컨테이너가 있는 {{site.data.keyword.containershort}}의 경우에는 {{site.data.keyword.Bluemix_notm}} UI에서도 이 서비스에 액세스할 수 있습니다. 
...
<table>
    <caption>표 1. {{site.data.keyword.monitoringshort}} 서비스의 플랜 요약</caption>
      <tr>
        <th>플랜</th>
      ...
      <tr>
        ...
        <td>사용할 수 없음</td>
        <td>15일</td>
        <td>최대 10개의 단일 메트릭 경보 규칙 또는 와일드카드를 포함하는 1개의 규칙을 정의할 수 있습니다. </td>
      </tr>
      <tr>
        <td>프리미엄</td>
        <td>사용 가능</td>
...
**참고:** 라이트 플랜은 {{site.data.keyword.Bluemix_notm}}의 통합 모니터링 기능과 동일한 기능을 제공합니다. 
...
최근 7일 동안 데이터를 수신하지 않은 메트릭은 삭제됩니다. {{site.data.keyword.monitoringshort}} 서비스는 지난 7일 동안 기록되지 않은 메트릭을 식별하여 임시적인 것으로 보이는 메트릭 경로의 모든 데이터를 삭제합니다. 예를 들면 다음과 같습니다. 
...
* {{site.data.keyword.Bluemix_notm}} 영역에서 실행 중인 클라우드 리소스에 대한 메트릭을 수집하려면 리소스가 실행 중인 영역에 서비스를 프로비저닝해야 합니다. 

* {{site.data.keyword.monitoringshort}} 서비스에 메트릭을 전송하려면 메트릭을 전송할 {{site.data.keyword.Bluemix_notm}} 영역에서 {{site.data.keyword.monitoringshort}} 서비스의 인스턴스를 프로비저닝해야 합니다.  
...
{{site.data.keyword.monitoringshort}} 서비스는 다음 지역에서 사용 가능합니다. 
...
{{site.data.keyword.monitoringshort}} 서비스는 {{site.data.keyword.Bluemix_notm}} 사용자 ID가 있으며 {{site.data.keyword.Bluemix_notm}} 조직의 영역에 대한 액세스 권한이 있는 모든 사용자가 사용할 수 있습니다.  
...
다음 표에는 지원되는 퍼블릭 클라우드 환경에 대한 URL이 나열되어 있습니다. 
